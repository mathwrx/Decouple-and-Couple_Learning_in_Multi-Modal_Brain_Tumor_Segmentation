...
def softmax_dice(output, target):
    ...
    '''
    out_target = F.one_hot(target, 4)
    out_target = out_target.permute(0, 4, 1, 2, 3).contiguous()

    dice_loss = tools.dice_loss(output, out_target, 4)
    ce_loss = tools.softmax_weighted_loss(output, out_target, 4)
